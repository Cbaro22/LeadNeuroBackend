...
export const handledeleteNurse = async (req, res) => {
        ...
        res.status(200).json({message: "Nurse deleted successfully", deletedNurse});
    } catch(error){
            next(error)
        }
    }
export const handlecreateNurse = async (req, res) => {
    try{
        ...
        if(!staff_id || !department || !shift || !supervisor || !workSchedule){
            ...
            res.status(201).json({message: "Nurse data created successfully", nurse})
        }
           catch(error){
            next(error)
        }
...
export const handlegetAllNurses = async (req, res) => {
    try{
        const nurses = await Nurse.find().populate("staff", "name email").lean();
        res.status(200).json({Message:"List of nurses", nurses});
    } catch(error){
        next(error)
    }
...
export const handlegetNurseById = async (req, res) => {
    try {
        const {id:staff_id} = req.params;  
